...
description: >
  ...
  users through hands-on analysis or structured lessons using Arkana's 284
...
2. **NO script writing**: Do NOT write Python scripts, one-liners, shell scripts,
   ...
   analysis. Arkana has 284 MCP tools that cover these operations — use them.
...
2. All tools are registered at startup. If `--brief-descriptions` is enabled,
   tool descriptions are shorter but all tools remain callable.
...
- [tooling-reference.md](../arkana-analyze/tooling-reference.md) — Complete
  259-tool catalog with "Use When" and "Prefer/Avoid" guidance
