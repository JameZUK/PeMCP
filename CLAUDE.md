...
Pages: Overview (binary summary, risk, mitigations, recent notes), Functions (sortable, triage buttons, enrichment score column, XREF analysis panel with clickable navigation, inline notes), Call Graph (Cytoscape.js with dagre layout, tabbed sidebar: INFO/XREFS/STRINGS/CODE, score-based border thickness), Sections (permission flags), Imports (DLL/export tables), Strings (FLOSS detail panel, type/category filtering, sifter scores), Timeline (expandable tool calls), Notes (category filtering).
