...
└── mcp/                # MCP tool modules (259 tools across 57 files)
...
- **Emulation debugger**: `tools_debug.py` provides 29 MCP tools for interactive step-through debugging via a persistent Qiling subprocess (`scripts/debug_runner.py`). Unlike fire-and-forget Qiling tools, debug sessions persist across multiple MCP calls using JSONL over stdin/stdout. Key classes: `_DebugSessionManager` (session lifecycle, eviction) and `_DebugSession` (subprocess handle, JSONL IPC). State stored in `state._debug_manager` (lazily created). Session reaper cleans up debug sessions via `cleanup_all()`. Tools: `debug_start`/`debug_stop`/`debug_status` (lifecycle), `debug_step`/`debug_step_over`/`debug_continue`/`debug_run_until` (execution), `debug_set_breakpoint`/`debug_set_watchpoint`/`debug_remove_breakpoint`/`debug_remove_watchpoint`/`debug_list_breakpoints` (breakpoints/watchpoints), `debug_read_state`/`debug_read_memory`/`debug_write_memory`/`debug_write_register` (inspection), `debug_snapshot_save`/`debug_snapshot_restore`/`debug_snapshot_list`/`debug_snapshot_diff` (snapshots), `debug_set_input`/`debug_get_output` (I/O stubs), `debug_get_api_trace`/`debug_clear_api_trace`/`debug_set_trace_filter` (API tracing), `debug_search_memory` (memory search), `debug_stub_api`/`debug_list_stubs`/`debug_remove_stub` (user-defined API stubs). **CRT stubs** (`stub_crt=True` default on `debug_start`): hooks ~47 Windows APIs needed for MSVC CRT initialization (GetSystemTimeAsFileTime, GetCurrentProcessId, GetProcessHeap, QueryPerformanceCounter, critical sections, TLS/FLS, EncodePointer, GetModuleHandle, etc.) via a data-driven registry with generic stub factory. Three stub types: simple return value, pointer-write + return, and custom handlers. Install order: CRT stubs → I/O stubs (override CRT for console APIs) → API trace hooks (lowest priority). **I/O stubs** (`stub_io=True` default on `debug_start`): hooks Win32 console APIs (GetStdHandle, WriteConsoleA/W, ReadConsoleA, SetConsoleMode, GetConsoleMode, AllocConsole, FreeConsole) to prevent UC_ERR_READ_UNMAPPED crashes from printf/cout/cin; captures output text and consumes from input queue. **User-defined stubs** (`debug_stub_api`): runtime-extensible API stubbing for any Windows API not covered by builtin stubs. Validates api_name (alphanum+underscore, max 128), return_value (hex/decimal/void), and optional writes (param_index + data_hex). `debug_list_stubs` shows all three categories (builtin I/O, builtin CRT, user). `debug_remove_stub` only removes user-defined stubs. **API tracing** (enabled by default): wildcard ENTER/EXIT hooks log all Windows API calls with args and return values; paginated retrieval with optional filter. **Memory search**: searches all mapped regions for string (UTF-8 + UTF-16LE) or hex patterns (with `??` wildcards). Constants: `MAX_DEBUG_SESSIONS` (3), `DEBUG_SESSION_TTL` (1800s), `DEBUG_COMMAND_TIMEOUT` (300s), `MAX_DEBUG_SNAPSHOTS` (10), `MAX_DEBUG_INSTRUCTIONS` (10M), `MAX_DEBUG_MEMORY_READ` (1MB), `MAX_DEBUG_BREAKPOINTS` (100), `MAX_DEBUG_WATCHPOINTS` (50), `MAX_DEBUG_CAPTURED_OUTPUT` (10K), `MAX_DEBUG_PENDING_INPUT` (1K), `MAX_DEBUG_API_TRACE` (10K), `MAX_DEBUG_SEARCH_MATCHES` (100), `DEBUG_SEARCH_CONTEXT_BYTES` (32), `MAX_DEBUG_USER_STUBS` (200), `MAX_DEBUG_STUB_WRITE_SIZE` (1024), `MAX_DEBUG_STUB_WRITES` (8).
