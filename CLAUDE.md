...
Arkana is a Model Context Protocol (MCP) server exposing 264 binary analysis tools to AI clients. It supports PE, ELF, and Mach-O formats with integrations for angr, capa, FLOSS, YARA, Binary Refinery, Qiling, and Speakeasy.
...
├── resource_monitor.py  # Process-level RSS/CPU monitoring (psutil daemon thread)
    └── mcp/                # MCP tool modules (264 tools across 58 files)
...
- **Resource monitor**: `arkana/resource_monitor.py` provides process-level memory (RSS) and CPU monitoring via a `psutil` daemon thread polling every `RESOURCE_MONITOR_INTERVAL` (10s, env: `ARKANA_RESOURCE_MONITOR_INTERVAL`). `get_resource_snapshot()` returns current RSS/CPU/thread count with `memory_level` (`normal`/`high`/`critical`). `get_resource_alert()` returns an alert dict when RSS exceeds `RESOURCE_MEMORY_HIGH_MB` (4096, env: `ARKANA_RESOURCE_MEMORY_HIGH_MB`) or `RESOURCE_MEMORY_CRITICAL_MB` (8192, env: `ARKANA_RESOURCE_MEMORY_CRITICAL_MB`). Alerts are injected into `_collect_background_alerts()` so the AI sees resource pressure in every tool response. `get_resource_usage` MCP tool provides on-demand snapshot with trend data. Dashboard global status bar shows MEM/CPU/THR indicators with severity-based badge colors. All functions return `None` when psutil is unavailable (`PSUTIL_AVAILABLE` guard). `RESOURCE_HISTORY_SIZE` (60) snapshots retained for trend analysis.
- **Null-region detection**: `detect_null_regions` MCP tool scans the loaded binary for contiguous null-byte regions (>= 512 bytes). Null bytes are interpreted by angr as `add [rax], al` (x86/x64), creating fake CFG functions. `_is_null_region_artifact()` in `tools_angr_disasm.py` filters these from `get_function_map` and the enrichment decompile sweep. `decompile_function_with_angr` warns when targeting a null-artifact function. `release_angr_memory` MCP tool drops the angr project/CFG and calls `gc.collect()` + `malloc_trim(0)` while preserving PE data, notes, and session state. Constant: `CFG_NULL_REGION_MIN_SIZE` (512).
- **Background task timeout**: All 14 background tools (`find_path_to_address`, `emulate_function_execution`, `analyze_binary_loops`, `get_reaching_definitions`, `get_data_dependencies`, `get_value_set_analysis`, `diff_binaries`, `find_path_with_custom_input`, `emulate_with_watchpoints`, `identify_cpp_classes`, `find_similar_functions`, `build_function_signature_db`, `solve_constraints_for_path`, `explore_symbolic_states`) use progress-adaptive timeouts via `_run_background_task_wrapper`. `TASK_OVERTIME` status: after the soft timeout (`BACKGROUND_TASK_SOFT_TIMEOUT`, 300s; `ANGR_CFG_SOFT_TIMEOUT`, 900s for CFG), tasks transition to `overtime` status (still running). During overtime, progress is checked every `OVERTIME_CHECK_INTERVAL` (60s) — if making progress, the task keeps running; if stalled for `OVERTIME_STALL_KILL` (300s), the task is killed. `OVERTIME_MAX_RUNTIME` (21600s / 6h) provides an absolute ceiling. All configurable via env vars (`ARKANA_ANGR_CFG_SOFT_TIMEOUT`, `ARKANA_BACKGROUND_TASK_SOFT_TIMEOUT`, `ARKANA_OVERTIME_CHECK_INTERVAL`, `ARKANA_OVERTIME_STALL_KILL`, `ARKANA_OVERTIME_MAX_RUNTIME`). Set soft timeout to 0 to fall back to old single hard-timeout behavior. When `soft_timeout` is not explicitly passed, the per-tool `timeout` value is used as the soft timeout before falling back to the env var default. `_background_alerts` are injected into every tool response when tasks are RUNNING (>5s) or in OVERTIME, giving the AI passive visibility into all active background work (enrichment, CFG builds, symbolic execution, etc.). `abort_background_task(task_id)` (264 tools total) explicitly stops any running/overtime task. `cancel_all_background_tasks()` on `AnalyzerState` marks all RUNNING/OVERTIME tasks as FAILED (with `aborted=True`) and sets all cancel events — called by `open_file`/`close_file` during file switch cleanup to ensure no stale tasks persist. Also called in `open_file`'s error handler. Generation guard (`state._analysis_generation`) prevents stale background threads from writing results after file switches; gen-mismatch early returns now mark tasks FAILED (not silently dropped). The overtime loop checks `cancel_event.is_set()` (not the `aborted` dict flag) for more reliable cancellation. `_update_progress()` skips updates when the task is no longer active (FAILED/COMPLETED). Enrichment thread is tracked in standard `_background_threads`/`_task_cancel_events` infra (reuses `_enrichment_cancel`), so `abort_background_task("auto-enrichment")` works reliably. BSim tools use `BSIM_BACKGROUND_TIMEOUT` (1800s, overridable via `ARKANA_BSIM_BACKGROUND_TIMEOUT`). Eight tools support `on_timeout` callbacks that capture partial results. Tasks include `created_at_epoch` for elapsed time reporting.
...
- **Emulation debugger**: `tools_debug.py` provides 29 MCP tools for interactive step-through debugging via a persistent Qiling subprocess (`scripts/debug_runner.py`). JSONL IPC over stdin/stdout. Key classes: `_DebugSessionManager` (lifecycle, eviction) and `_DebugSession` (subprocess handle). State stored in `state._debug_manager` (lazily created). Tool groups: lifecycle (start/stop/status), execution (step/step_over/continue/run_until), breakpoints/watchpoints (set/remove/list), inspection (read_state/read_memory/write_memory/write_register), snapshots (save/restore/list/diff), I/O (set_input/get_output), API tracing (get/clear/set_filter), memory search, and user-defined stubs (stub_api/list_stubs/remove_stub). Three stub layers installed in order: CRT stubs (~47 MSVC CRT APIs), I/O stubs (Win32 console APIs), then API trace hooks. All use `hook_address()` per IAT entry (see Known Tool Limitations for rationale). See Input Validation section for session/resource limits.
...
**CSP compliance**: Dashboard uses `script-src 'self'` and `img-src 'self'` CSP (no `data:` URIs, no inline scripts). All event handlers use `addEventListener` or event delegation — no inline `onclick` attributes. Dynamic HTML uses `data-*` attributes for actions with delegated listeners.
...
- **Dedicated dashboard thread pool**: All dashboard API endpoints and htmx partials run data functions via `_dash_to_thread()` — a drop-in replacement for `asyncio.to_thread()` that uses a dedicated `ThreadPoolExecutor` (`_dashboard_executor`, 4 threads by default, env: `ARKANA_DASHBOARD_THREADS`). This prevents dashboard requests from being starved when MCP tools or background tasks saturate the default executor. Partials include `try/except` with empty-HTML fallbacks.
...
- **Binary Diff**: `/api/diff` runs angr BinDiff via `_dash_to_thread()`. `/api/list-files` lists samples directory for the file browser. Uses `getattr()` with fallback for BinDiff attribute names across angr versions.
...
- **Debug sessions**: Emulation fidelity limited by Qiling/Unicorn — anti-emulation techniques, threading, and complex Windows APIs may not work. Memory watchpoints have performance overhead (global hook per type). Snapshots consume memory proportional to mapped regions. `ql.save()`/`ql.restore()` may not fully restore complex states (file handles, network sockets). CRT stubs provide minimal fidelity (fake timestamps, fake PID, identity EncodePointer) sufficient for CRT init but not for application logic depending on real values. Uses `hook_address()` per IAT entry instead of `set_api()` to avoid Qiling's forwarded-API double-hook issue on x86 STDCALL — API breakpoints can now be cleanly removed via `hook_del()`. **Register writes do not redirect execution** — Unicorn's `emu_start()` uses its own start address and ignores EIP/RIP changes via `debug_write_register`; use code patching (`debug_write_memory` with JMP/`EB FE` instructions) instead. **Unresolved MSVCRT imports** (e.g., `_initterm_e`, `_initterm`) leave IAT entries pointing to invalid addresses — patch IAT entries to point to a `xor eax,eax; ret` code-cave stub. **Threading** is unsupported — stub `CreateThread`/`WaitForMultipleObjects` and redirect main execution into thread functions via code patching. **Timeout behaviour**: Execution commands (step/continue/run_until/step_over) pass `timeout_seconds` to the runner subprocess which runs `_ql.run()` in a daemon thread and calls `_ql.emu_stop()` (Unicorn's thread-safe stop) on timeout. The session is **paused, not killed** — memory, registers, and hooks are fully preserved. Use `debug_read_state`/`debug_read_memory`/`debug_search_memory` to inspect the paused state, then `debug_continue` to resume from exactly where it stopped. `DEBUG_COMMAND_TIMEOUT` (300s, env: `ARKANA_DEBUG_COMMAND_TIMEOUT`) controls the runner-side deadline; the client-side `asyncio.wait_for` timeout adds `DEBUG_RUNNER_TIMEOUT_BUFFER` (15s) as a last-resort safety net.
...
- **`detect_null_regions`**: Scans raw file bytes for null runs — may flag legitimate null-initialized data sections. Classification is heuristic-based (section flags, position).
- **`release_angr_memory`**: After release, any angr-dependent tool will rebuild project and CFG from disk. Re-analysis time depends on binary size.
