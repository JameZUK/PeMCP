...
async def map_mitre_attack(
    ...
    def _map():
        # M7-v9: Delegate to _map_mitre_internal to eliminate ~100 lines of duplication
        from arkana.state import get_current_state
        result = _map_mitre_internal(get_current_state())
        # Optionally build Navigator layer (only addition over internal version)
        if include_navigator_layer:
            result["navigator_layer"] = _build_navigator_layer(result["techniques"])
        return result
